...
public abstract class PlumRequester implements IMessageSender {
    ...
    protected void execute() throws DBException {
        try {
            ...
            for (int reqID = 0; reqID < this.requests.size(); reqID++) { // Execute for each request
                ...
                // Preparing req
                if (_co.getConfig().doDBLog()) sendMessage(IMessageSender.CONSOLE, String.format(DBMessages.PREPARE_REQ.getMessage(), r),PrefixOption.NO_PREFIX);
                ...
                // Executing req
                if (_co.getConfig().doDBLog()) sendMessage(IMessageSender.CONSOLE, DBMessages.EXECUTING_REQ.getMessage(), PrefixOption.NO_PREFIX);
                req.execute();
